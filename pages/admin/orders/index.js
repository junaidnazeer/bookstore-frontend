    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl shadow-lg p-6 w-full max-w-sm">
        ...
        <button
          onClick={handleSave}
          disabled={saving}
          className="w-full py-2.5 bg-spine text-white rounded-lg font-medium mb-2 hover:opacity-90 transition-opacity disabled:opacity-50"
        >
          {saving ? "Saving..." : "Save Stock"}
        </button>
        <button
          onClick={onClose}
          className="w-full py-2.5 border border-neutral-300 rounded-lg text-ink hover:bg-neutral-50 transition-colors"
        >
          Cancel
        </button>
      </div>
  ...
  return (
    <AdminLayout>
      ...
      <div className="flex gap-3 mb-4">
        <div className="flex items-center border border-neutral-300 rounded-lg px-3 flex-1 bg-white">
                ...
                <tr key={p.id} className="border-b border-neutral-100">
                  ...
                  <td className="p-3 flex gap-2 flex-wrap">
                    <button
                      ...
                      className="px-3 py-1.5 border border-neutral-300 rounded-lg text-xs hover:bg-neutral-50 transition-colors"
                    >
                      Restock
                    </button>
                    <Link
                      href={`/admin/products/${p.id}/edit`}
                      className="px-3 py-1.5 border border-spine text-spine rounded-lg text-xs hover:bg-spine/5 transition-colors"
                    >
                      Edit
                    </Link>
                    <button
